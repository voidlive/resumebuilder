...
export const ResumeBuilder: React.FC = () => {
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      if (!response.ok || response.headers.get('Content-Type') !== 'application/pdf') {
        let errorDetails = `Server failed with status: ${response.status}`;
        try {
            // Try to parse a JSON error message from the server
            const errorJson = await response.json();
            errorDetails = errorJson.error || errorJson.message || errorDetails;
        } catch (e) {
            console.warn("Could not parse server error response as JSON. It might be an HTML error page.");
        }
      ...
      setNotification({
        message: "High-quality PDF server failed. Falling back to an in-browser PDF. This may have lower quality.",
        type: 'info'
      });
  ...
  return (
    <>
      ...
      <div className="font-inter text-zinc-800">
        <header className="bg-white shadow-sm p-4 sticky top-[73px] z-20 no-print border-t border-b">
